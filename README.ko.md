...
OpenConveneCLI는 9개의 AI 코딩 에이전트 CLI를 즉시 지원합니다. 각 CLI는 자체 모델 백엔드에 연결됩니다 — OpenConveneCLI 자체는 어떤 클라우드 서비스에도 의존하지 않습니다. 도구를 사용하려면 최소 1개의 CLI가 설치되어 있어야 합니다.

| CLI | 설명 | 읽기 전용 | 실행자 | 설치 명령어 |
|-----|------|-----------|----------|-----------------|
| [Devin](https://devin.ai) | Cognition의 자율형 AI 소프트웨어 엔지니어. 셸 액세스, 브라우저 제어, 장기 작업 계획 기능을 갖춘 풀스택 코딩 에이전트. | 가능 | 예 | `curl -fsSL https://cli.devin.ai/install.sh \| bash` |
| [Grok](https://x.ai) | xAI의 AI 코딩 CLI (Grok 모델 탑재). 빠른 추론과 코드 생성, 실시간 지식 접근. | 가능 | 예 | `curl -fsSL https://x.ai/cli/install.sh \| bash` |
| [Codex](https://github.com/openai/codex) | OpenAI의 터미널 기반 코딩 에이전트. 샌드박스 실행——`--sandbox read-only`로 안전한 연구, `workspace-write`로 코드 실행. | 예 | 예 | `npm install -g @openai/codex` |
| [Antigravity / agy](https://antigravity.google) | Google의 AI 코딩 에이전트 CLI (Gemini 탑재). 다중 파일 편집, 코드 리뷰, 에이전트 작업 실행 (Gemini 2.5 Pro/Flash). | 가능 | 예 | `curl -fsSL https://antigravity.google/cli/install.sh \| bash` |
| [Cursor](https://cursor.com) | AI 우선 코드 에디터 (에이전트 모드 탑재). `--force` 없이 읽기 전용 분석, `--force`로 자율 파일 편집. Claude, GPT-4, Gemini 탑재. | 예 | 예 | `curl https://cursor.com/install -fsS \| bash` |
| [Kimi Code](https://code.kimi.com) | Moonshot AI의 코딩 CLI (Kimi K2 탑재). 긴 문맥 코드 이해 (256K 토큰), 읽기 전용 작업 자동 승인. | 예 | 예 | `curl -fsSL https://code.kimi.com/kimi-code/install.sh \| bash` |
| [Hermes](https://github.com/hashicorp/hermes) | HashiCorp의 AI 에이전트 CLI. `chat -q`로 단일 쿼리 모드, 에이전트 모드로 다단계 인프라·코드 작업. | 가능 | 예 | `hermes setup --portal` |
| [Aider](https://aider.chat) | 오픈소스 AI 페어 프로그래밍 도구. Git 통합, GPT-4o, Claude 3.5, DeepSeek, 로컬 LLM 지원. 편집 우선 설계——기본적으로 파일 수정. | 아니오 | 예 | `python -m pip install aider-install && aider-install` |
| [OpenCode](https://opencode.ai) | 오픈소스 AI 코딩 에이전트. `run` 서브명령으로 비대화형 단일 프롬프트, 에이전트 모드로 자율 개발. 다중 LLM 프로바이더 지원. | 가능 | 예 | [opencode.ai/docs/cli](https://opencode.ai/docs/cli/) 참조 |

> **읽기 전용**은 해당 CLI가 응답자 모드에서 안전하게 동작할 수 있는지(파일 수정 없음)를 나타냅니다. `예` = 읽기 전용 강제, `가능` = 비대화형 모드이나 도구 호출을 트리거할 수 있음, `아니오` = 기본적으로 파일 수정 (실행자 전용).
