...
OpenConveneCLI は 9 つの AI コーディングエージェント CLI に標準で対応しています：

| CLI | 読み取り専用 | エグゼキューター | インストールコマンド |
|-----|-----------|----------|-----------------|
| [Devin](https://devin.ai) | はい | はい | `curl -fsSL https://cli.devin.ai/install.sh \| bash` |
| [Grok](https://x.ai) | はい | はい | `curl -fsSL https://x.ai/cli/install.sh \| bash` |
| [Codex](https://github.com/openai/codex) | はい | はい | `npm install -g @openai/codex` |
| [Antigravity (agy)](https://antigravity.google) | はい | はい | `curl -fsSL https://antigravity.google/cli/install.sh \| bash` |
| [Cursor](https://cursor.com) | はい | いいえ | `curl https://cursor.com/install -fsS \| bash` |
| [Kimi Code](https://code.kimi.com) | はい | はい | `curl -fsSL https://code.kimi.com/kimi-code/install.sh \| bash` |
| [Hermes](https://github.com/hashicorp/hermes) | はい | はい | `hermes setup --portal` |
| [Aider](https://aider.chat) | はい | はい | `python -m pip install aider-install && aider-install` |
| [OpenCode](https://opencode.ai) | はい | はい | [opencode.ai/docs/cli](https://opencode.ai/docs/cli/) を参照 |

> 各 CLI は独自のモデルバックエンドに接続します。OpenConveneCLI 自体はクラウドサービスに依存しません。
